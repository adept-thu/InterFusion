            ...
            if has_label:
                ...
                if count_inside_pts:
                    if self.pc_type == 'lidar' or self.pc_type == 'radar':
                        ...
                        calib = self.get_calib(sample_idx)
                        lidar_points_R = lidar_points[:, :3]
                        lidar_points_ones = np.ones(len(lidar_points))
                        lidar_points_R = np.c_[lidar_points_R, lidar_points_ones]
                        lidar_points[:, :3] = np.dot(calib['T_from_lidar_to_radar'], lidar_points_R.T).T[:, :3]
