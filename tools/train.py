import argparse                   # parse command line parameters
import datetime                   # get the time string
...
def parse_config():
    ...
    # command line options, and parsing of parameters
    # read "help" to learn the definition of each attributes
    ...
    # return all added attributes to the args subclass instance
    args = parser.parse_args()

    # override the default settings in the yaml file with the new attributes
    cfg_from_yaml_file(args.cfg_file, cfg)
    cfg.TAG = Path(args.cfg_file).stem
    cfg.EXP_GROUP_PATH = '/'.join(args.cfg_file.split('/')[1:-1])  # remove 'cfgs' and 'xxxx.yaml'
    
    # assign the data in args.set_cfgs to the corresponding key name
...
def main():
    ...
    # init train env first, since logger depends on the dist info
    ...
    # set the corresponding batch size according to whether distributed training is performed or not
    ...
    # select the number of epochs that should be used based on the parameter settings or default values
    args.epochs = cfg.OPTIMIZATION.NUM_EPOCHS if args.epochs is None else args.epochs
    
    # set a fixed random number to ensure that the experiment can be reliably reproduced
    ...
    # set the path of the checkpoint file output
    ...
    # write the args argument to the log
    for key, val in vars(args).items():
        logger.info('{:16} {}'.format(key, val))
    log_config_to_file(cfg, logger=logger)
    
    # setting local process number option
    if cfg.LOCAL_RANK == 0:
        os.system('cp %s %s' % (args.cfg_file, output_dir))
    
    # setting up the tensorboard file
    tb_log = SummaryWriter(log_dir=str(output_dir / 'tensorboard')) if cfg.LOCAL_RANK == 0 else None

    # -----------------------create dataloader & network & optimizer---------------------------
    # load the model's data by setting the dataloader
    ...
    if args.ckpt is not None:
        it, start_epoch = model.load_params_with_optimizer(args.ckpt,
                            to_cpu=dist, optimizer=optimizer, logger=logger)
        last_epoch = start_epoch + 1
    else:
        # Returns the list of all matching file paths via the glob.glob() function
    ...
    # set the learning rate, and Learning rate about warmup
    ...
    # train the model based on the already well-defined model
    train_model(
        model,
        optimizer,
        train_loader,
